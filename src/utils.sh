...
function generate_active_window_string() {
  ...
  if [ "$transparent" = "true" ]; then
    ...
    separator_start="#[bg=default,fg=${PALLETE['red']}]${left_separator_inverse}#[bg=${PALLETE['red']},fg=${PALLETE['bg_highlight']}]"
    separator_internal="#[bg=${PALLETE['orange']},fg=${PALLETE['red']}]${left_separator:?}#[none]"
    separator_end="#[bg=default,fg=${PALLETE['orange']}]${left_separator:?}#[none]"
  else
    separator_start="#[bg=${PALLETE['red']},fg=${PALLETE['bg_highlight']}]${left_separator:?}#[none]"
    separator_internal="#[bg=${PALLETE['orange']},fg=${PALLETE['red']}]${left_separator:?}#[none]"
    separator_end="#[bg=${PALLETE[bg_highlight]},fg=${PALLETE['orange']}]${left_separator:?}#[none]"
  fi

  echo "${separator_start}#[fg=${PALLETE[terminal_black]}]#I${separator_internal}#[fg=${PALLETE[terminal_black]}] #{?window_zoomed_flag,$zoomed_window_icon,$active_window_icon}${active_window_title}#{?pane_synchronized,$pane_synchronized_icon,}${separator_end}#[none]"
}
